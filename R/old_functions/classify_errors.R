...
#' @param classify_also_RAs A logical value indicating whether to classify rows with `RA` equal to 1 according to the conditions (default is FALSE). If FALSE, such rows will have the classify_errors results set to NA.
#'
...
#' - `human_check`: A column indicating "required" if the sum of the classify_errors columns (from `lexicalization` to `no_response`) plus the `correct` column is 0 or greater than or equal to 2 for that row; otherwise, it is empty.
#'
#' When `classify_RAs` is FALSE, rows where `RA` is 1 will have the classify_errors results set to NA. When `classify_RAs` is TRUE, classify_errors is performed normally regardless of the `RA` value.
#'
#' @returns A data frame with additional columns for each classify_errors category: `lexicalization`, `nonword`, `neologism`, `formal`, `unrelated`, `morphological`, `mixed`, and `semantic`. Each column contains binary values indicating the classify_errors status of each response, along with `no_response` and `human_check` columns.
#'
#' @export
classify_errors = function(dataframe, access_col = "accessed", RA_col = "RA",
                           cosine_limit_value = 0.46, response_col = "response_col",
                           classify_also_RAs = FALSE) {
  ...
  dataframe = dataframe %>%
    ...
    mutate(
      ...
      #      morphological = ifelse( # this error will be included in posterior versions
      #        correct != 1 &
      #          same_root == 1 &
      #          is_target_nonword != 1 &
      #          is_plural == 1,
      ...
      # Set errors to NA if RA is 1 unless classify_also_RAs is TRUE
      across(c(lexicalization, nonword, neologism, formal, unrelated,
               #morphological,
               mixed, semantic, no_response),
             ~ ifelse(!classify_also_RAs & RA == 1, NA, .)),
      # New column for human check
      human_check = case_when(
        rowSums(across(c(lexicalization, nonword, neologism, formal, unrelated,
                         #morphological,
                         mixed, semantic, no_response)), na.rm = TRUE) + correct >= 2 ~ "required",
        rowSums(across(c(lexicalization, nonword, neologism, formal, unrelated,
                         #morphological,
                         mixed, semantic, no_response)), na.rm = TRUE) + correct == 0 ~ "required",
        TRUE ~ ""
      )
    )
